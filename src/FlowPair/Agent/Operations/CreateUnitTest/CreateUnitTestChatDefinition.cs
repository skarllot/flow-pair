...
{
    public const string JsonResponseKey = "CreateUnitTestResponse";
    ...
    public Result<object, string> Parse(string key, string input) => key switch
    {
        JsonResponseKey => ContentDeserializer
            .TryDeserialize(input, jsonContext.CreateUnitTestResponse)
    ...
    public Option<CreateUnitTestResponse> ConvertResult(ChatWorkspace chatWorkspace) =>
        OutputProcessor.GetFirst<CreateUnitTestResponse>(chatWorkspace, JsonResponseKey);
}
